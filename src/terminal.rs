...
use pancurses::{
    chtype, curs_set, def_prog_mode, def_shell_mode, doupdate, echo, endwin, init_pair, initscr,
    noecho, noraw, raw, reset_prog_mode, reset_shell_mode, resize_term, start_color, Window,
    COLOR_BLACK, COLOR_BLUE, COLOR_CYAN, COLOR_GREEN, COLOR_MAGENTA, COLOR_PAIR, COLOR_RED,
    ...
    pub fn move_print(&self, mut x: i32, mut y: i32, s: impl Into<ColorString>) {
        ...
        for ch in cs.to_string().chars() {
            if ch == '\n' {
                ...
                x += 1;
                self.win.mvprintw(y, x, ch.to_string());
            }
        }
    }
